...
```{r}
# # PORIFERA
# Taxonary[Taxonary$Reference_List %in% c("Porifera encrusting;bluegrey", "Porifera encrusting;brown","Porifera encrusting;green","Porifera encrusting;grey","Porifera encrusting;orange","Porifera encrusting;purple", "Porifera encrusting;red","Porifera encrusting;white", "Porifera encrusting;yellow"), 'Analysis_name']<-"Porifera encrusting" #Haemedesmia
# Taxonary[Taxonary$Reference_List %in% c("Porifera medium;white","Porifera medium;yellow"), 'Analysis_name']<-"Porifera medium"
# Taxonary[Taxonary$Reference_List %in% c("Porifera small;irregular","Porifera small;orange","Porifera small;white","Porifera small;yellow", "Porifera small;spikey", "Porifera small;green", "Grantia compressa"), 'Analysis_name']<-"Porifera small"
# 
# 
# Taxonary[Taxonary$Reference_List %in% c("Porifera small;round yellow",  "Porifera_window", "Tetilla sp."), 'Analysis_name']<-"Porifera small round"
# 
# Taxonary[Taxonary$Reference_List %in% c("Porifera_cupcakes"), 'Analysis_name']<- "Tentorium semisuberites"
# 
# otuCompl$Porifera_cupcake >0
# 
...
FixThis<-c("Primnoa resedaeformis" ,"Paragorgia arborea", "Nephtheidae")
...
```{r}
Traits_orig<-read_excel("~/Input/2024/Sub_VideoTraits.xlsx", col_names = TRUE)

otu<-otu[,which(!(colnames(otu) %in% c("Actiniaria_buried_redish", "Actiniaria_yellow", "Cerianthidae_violet", "Cerianthidae_dark")))]
...
TraitsSel<-Traits_orig[Traits_orig$Reference_List2 %in% colnames(otu),]


TraitsSel<-TraitsSel[,c('AphiaID', 'Reference_List2',
                      #  names(TraitsSel)[12:28],
              ...
              # 'Reprod_AsexualBudding','Reprod_Sexual_BroadcastSpawnEggs','Reprod_Sexual_SeaBedEggs','Reprod_Sexual_BroodsEggs','Reprod_Hermaphrodite',
                ...
                #'Sensitivity_Fragile.shell.structure.','Sensitivity_NoProtection','Sensitivity_ToughSkin_Exoskeleton','Sensitivity_Regeneration', Its wrong! look at ceryanthus for example
                #'HabitatEffect_ReefBuilding','HabitatEffect_Bedforming')
                ]
#write.xlsx(TraitsSel, "Sub_VideoTraits.xlsx", sheetName = "Traits",col.names = TRUE, row.names = FALSE)
#TraitsSel<-read_excel("Sub_VideoTraits.xlsx", col_names = TRUE)
...
TraitsSel[TraitsSel$Size %in% c("Size_0_1","Size_1_2", "Size_2_10"),'Size']<-"1"
TraitsSel[TraitsSel$Size %in% c("Size_10_20"),'Size']<-"2"
TraitsSel[TraitsSel$Size %in% c("Size_20_50"),'Size']<-"3"
TraitsSel[TraitsSel$Size %in% c("Size_50cm_"),'Size']<-"4"
TraitsSel$Size<-as.numeric(paste(TraitsSel$Size))
...
traits<-TraitsSel#[,c(1,2, 43:ncol(TraitsSel))]

# Feeding_Categ <- TraitsSel %>%
#   summarise(Feed_SurfaceDeposit = pmax(Feed_SurfaceDeposit, Feed_SubsurfaceDeposit, na.rm = TRUE),
#             Feed_PredScav = pmax(Feed_Scavenger, Feed_Predator, na.rm = TRUE),
#             Feed_SuspensionFilter = Feed_SuspensionFilter)
# traits<-cbind(traits, Feeding_Categ)

rownames(traits)<-traits$Reference_List2
#traits$Phylum<-Taxonomic_DF$Phylum
...
# TRAITS
```{r}
Traits_orig<-read_excel(paste0(PathInputData,"/Sub_VideoTraits2.xlsx"), col_names = TRUE)
