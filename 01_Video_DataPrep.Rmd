...
source("/gpfs/gpfs0/home/home2/a39495/MY_TaxonomicFromWormsDF.r")
```
...
VME species:
# I add this to make sure VME species are kepts even if they are below "MinSightings"
```{r}
source("/gpfs/gpfs0/home/home2/a39495/VME_Spp.R")
```
...
```{r, warning=FALSE}
if(FALSE){
FishingEffort<-read.csv("/gpfs/gpfs0/home/home2/a39495/Input/effort_fish.csv", sep = ",")
ShrimpingEffort<-read.csv("/gpfs/gpfs0/home/home2/a39495/Input/effort_shrimp.csv", sep = ",")
...
load("/gpfs/gpfs0/home/home2/a39495/Input/rHarvestEffort_raster.rda") # loads rHarvestEffort
```

## -Env
```{r, warning=FALSE}
env_orig <- read.csv("/gpfs/gpfs0/home/home2/a39495/Input/2024/envData_2023-07-21.csv") %>% as.data.frame
env_orig<-unique(env_orig)
env_orig[sapply(env_orig, is.character)] <- lapply(env_orig[sapply(env_orig, is.character)], as.factor)

samp_info <- read.csv("/gpfs/gpfs0/home/home2/a39495/Input/2024/sample_info.csv") %>% as.data.frame
...
## -Bio
```{r, warning=FALSE}
otu_orig <- read.csv("/gpfs/gpfs0/home/home2/a39495/Input/2024/species_densities.csv") %>% as.data.frame
...
## - Taxon
```{r, warning=FALSE}
Taxonary<-read.xlsx("~/Input/2024/Taxonary_2023-11-20.xlsx", sheet = 1)
...
otu_orig<-otu_orig[,c(TRUE, colnames(otu_orig)[-1] %in% Taxonary[
  Taxonary$Size_class %in% c("OK", "Detailed") & 
    ...
    !(Taxonary$Grouping_taxon_biotope %in% c("Algae", "Bacteria", "Biota","Other worms", "Foraminifera"))  ,'Reference_List'])]
```
...
```{r}
#Takes otuCompl
source("/gpfs/gpfs0/home/home2/a39495/TaxonomicGroupings.r") 
...
FixThis<-c("Porifera encrusting","Porifera medium" ,"Porifera small","Porifera small round","Porifera small;stalked","Porifera small;stalked","Porifera erect","Porifera branched","Polymastiidae","Porifera fan","Primnoa resedaeformis" ,"Paragorgia arborea")
...
ggplot()+
  geom_point(data = env_orig[rowSums(env_orig[,c('no_vision','poor_vision')], na.rm = TRUE) > 0.5,], aes(x_coord, y_coord, color = "Poor vision"), alpha = 0.5, size = 0.8)+
  geom_point(data = env_orig[env_orig$SegLength< 190,], aes(x_coord, y_coord, color = "Too short"), alpha = 0.5, size = 0.8)+
  geom_point(data = env_orig[env_orig$SegLength> 300,], aes(x_coord, y_coord, color = "Too long"), alpha = 0.5, size = 0.8)+

  geom_point(data = env_orig[env_orig$bathy > -100,], aes(x_coord, y_coord, color = "Too shallow"), alpha = 0.5, size = 0.8)+
  geom_point(data = env_orig[env_orig$cruise_no  == "2006612",], aes(x_coord, y_coord, color = "2006 cruise"), alpha = 0.5, size = 0.8)+
  geom_point(data = envSel_split[["All"]], aes(x_coord, y_coord, color = "Not excluded"), size = 0.8)+
...
#env_orig[!(env_orig$VL %in% envSel_split[["All"]]$VL),]
...
save(Taxonomic_phylo, file = paste0("/gpfs/gpfs0/home/home2/a39495/temp/Taxonomic_phylo_",SPLIT,".rda"))
save(Branch_col, file = paste0("/gpfs/gpfs0/home/home2/a39495/temp/Branch_col_",SPLIT,".rda"))
save(Taxonomic_DF, file = paste0("/gpfs/gpfs0/home/home2/a39495/temp/Taxonomic_DF",SPLIT,".rda"))
...
save(otu, file = paste0("otu_Video_",SPLIT,".rda"))
save(env, file = paste0("env_Video_",SPLIT,".rda"))
```
