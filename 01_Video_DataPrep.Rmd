...
temp_01<-joinedDat[!(str_sub(joinedDat$SampID,-2,-1) %in% c("06","05","04","03","02","00" ,"_D")),]
joinedDat<-joinedDat[!(str_sub(joinedDat$SampID,-2,-1) %in% c("06","05","04","03","02","01", "_D")),]
...
joinedDat<-joinedDat[!(joinedDat$VL %in% c('2340', '544', '254', '829', StationRefs[!is.na(StationRefs$reason_excluded),'nSampleNumber'])),]
```

# SEPARATE joineddat
Separate the now matching datasets (joinedDat -> otu_sort + env_sort)
```{r, warning=FALSE}
otu_sort<- joinedDat %>% dplyr::select(c(1,2:which(colnames(joinedDat)==names(otu_orig)[ncol(otu_orig)])))

envCompl<-env_sort<-joinedDat %>% dplyr::select(c(1, (which(colnames(joinedDat)==names(otu_orig)[ncol(otu_orig)])+1):ncol(joinedDat)))
...
ggplot()+
  geom_point(data = env_orig[ env_orig$SampID %in% otuCompl[rowSums(otuCompl[, -c(1)]>0) <= MinOTUs,'SampID'],], aes(x_coord, y_coord, color = "Few otus"), size = 0.8)+
  ...
  geom_point(data = env_orig[env_orig$SegLength< length_lower,], aes(x_coord, y_coord, color = "Too short"), alpha = 0.5, size = 0.8)+
  geom_point(data = env_orig[env_orig$SegLength> length_upper,], aes(x_coord, y_coord, color = "Too long"), alpha = 0.5, size = 0.8)+

  geom_point(data = env_orig[env_orig$bathy > MinDepth,], aes(x_coord, y_coord, color = "Too shallow"), alpha = 0.5, size = 0.8)+
  geom_point(data = env_orig[env_orig$cruise_no  == "2006612" | (env_orig$cruise_no == "2019115" & env_orig$x_coord> 400000 & env_orig$y_coord>8500000),], aes(x_coord, y_coord, color = "2006 cruise \n& Kongfjord"), alpha = 0.5, size = 0.8)+
...
TraitsSel<-Traits_orig[,c('AphiaID', 'Reference_List2',
                'Sociability_Solitary', 'Sociability_Gregarious', 'Sociability_Colonial',"SociabilityGradient",
...
TraitsSel[TraitsSel$Habitat %in% c("Habit_BurrowDwelling" ,"Habit_CreviceDweller"),'Habitat']<-"BurrowORCrevice"
TraitsSel[TraitsSel$Habitat == "Habit_FreeLiving" ,'Habitat']<-"Free"
TraitsSel[TraitsSel$Habitat == "Habit_TubeDwelling" ,'Habitat']<-"TubeDwelling"
...
TraitsSel$Colonial<-ifelse(TraitsSel$Sociability_Colonial>0, "Col", "NonCol")
...
traits<-traits[match(otu_sp, row.names(traits)),]
```
